...
import { collection, getDocs, addDoc, deleteDoc, doc, setDoc, getDoc, updateDoc, query, where, deleteField } from "https://www.gstatic.com/firebasejs/12.19.0/firebase-firestore.js";
...
const SoalManager = {
    ...
    _makeTemplateWorkbook: async function(questions, summaryTitle = 'RINGKASAN BANK SOAL') {
        ...
        workbook.creator = 'CBT TRIGUNA 1956';
        workbook.company = 'TRIGUNA 1956 VOCATIONAL SCHOOL';
        workbook.subject = 'Bank Soal CBT';
        workbook.title = 'Bank Soal CBT TRIGUNA 1956';
    ...
    downloadSemua: async function(btnEl = null) {
        ...
        try {
            ...
            await this._downloadExcel(questions, `Bank_Soal_TRIGUNA_1956_Seluruh_Mapel.xlsx`, 'RINGKASAN BANK SOAL TRIGUNA 1956 - SELURUH MAPEL');
...
function handleRouting() {
    ...
    if ((hash === 'section-pengaturan' || hash === 'section-pengguna') && !isAdmin) hash = 'section-beranda';
...
function renderAuthProblem(title, message, showRetry) {
    if (document.getElementById("auth-problem-overlay")) return;
    const box = document.createElement("div");
    box.id = "auth-problem-overlay";
    box.style.cssText = "position:fixed;inset:0;background:rgba(15,23,42,.97);display:flex;align-items:center;justify-content:center;padding:22px;z-index:99999";
    box.innerHTML = `<div style="background:#fff;border-radius:18px;max-width:560px;width:100%;padding:30px;text-align:center;font-family:system-ui"><h2 style="margin:0 0 12px;color:#0f172a">${title}</h2><p style="margin:0 0 18px;color:#475569;line-height:1.6">${message}</p><div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap"><button id="auth-problem-reload" style="background:#0ea5e9;color:#fff;border:0;border-radius:10px;padding:12px 18px;cursor:pointer">Coba lagi</button>${showRetry ? `<button id="auth-problem-home" style="background:#e2e8f0;color:#0f172a;border:0;border-radius:10px;padding:12px 18px;cursor:pointer">Kembali Login</button>` : ""}</div></div>`;
    document.body.appendChild(box);
    document.getElementById("auth-problem-reload")?.addEventListener("click", () => location.reload());
    document.getElementById("auth-problem-home")?.addEventListener("click", () => location.replace("index.html"));
}
...
onAuthStateChanged(auth, async (user) => {
    ...
    try {
        ...
        console.error("Gagal memverifikasi role dashboard:", e);
        renderAuthProblem("Firestore belum dapat membaca profil", String(e?.code || e?.message || "Kesalahan Firestore"), false);
        return;
    }
    ...
    if (!isAdmin) {
        const menuPengguna = document.getElementById('menu-pengguna'); if (menuPengguna) menuPengguna.style.display = 'none';
        ...
        const menuPengguna = document.getElementById('menu-pengguna'); if (menuPengguna) menuPengguna.style.display = '';
    ...
    handleRouting();
    // Bootstrap dashboard setelah role terverifikasi. Admin otomatis melakukan recovery
    // data master dari data Firebase yang sudah ada agar Mapel/Kelas langsung muncul.
    await Promise.allSettled([
        window.loadDataMaster({ syncLegacy: isAdmin }),
        window.loadDataHasil(),
        SoalManager.loadSummary(),
        ...(isAdmin ? [window.loadDataPengguna()] : [])
    ]);
});
...
const normalizeMasterValues = (value) => {
    if (Array.isArray(value)) return value.flatMap(v => normalizeMasterValues(v));
...
const mergeMasterValues = (targetSet, value) => {
    normalizeMasterValues(value).forEach(v => targetSet.add(v));
};
...
window.loadDataMaster = async (options = {}) => {
    ...
    try {
        ...
        if (cached) applyAcademicMaster(cached.listMapel, cached.listKelas);
        ...
        if (docSnap.exists()) {
            ...
            currentMapel = normalizeMasterValues(data.list_mapel ?? data.mapel ?? data.mataPelajaran);
            currentKelas = normalizeMasterValues(data.list_kelas ?? data.kelas);
        }

        // Recovery otomatis untuk data yang sudah tersimpan pada akun/soal/hasil.
        // Hanya Admin yang menjalankan recovery koleksi users agar sesuai Firestore Rules.
        const needRecovery = isAdmin && (syncLegacy || !docSnap.exists() || !currentMapel.length || !currentKelas.length);
        if (needRecovery) {
            ...
            const mergedMapel = new Set(currentMapel);
            const mergedKelas = new Set(currentKelas);
            const absorb = (data) => {
                if (!data) return;
                mergeMasterValues(mergedMapel, data.mapel);
                mergeMasterValues(mergedMapel, data.mataPelajaran);
                mergeMasterValues(mergedKelas, data.kelas);
            };
            results.forEach(result => {
                if (result.status === 'fulfilled') result.value.forEach(d => absorb(d.data()));
                else console.warn('Sumber recovery master dilewati:', result.reason?.code || result.reason);
            });
            currentMapel = Array.from(mergedMapel);
            currentKelas = Array.from(mergedKelas);
            if (currentMapel.length || currentKelas.length) {
...
window.loadDataPengguna = async () => {
    ...
    try {
        ...
        allUsersData.sort((a, b) => {
            const roleA = (Array.isArray(a.role) ? a.role : [a.role]).map(r => String(r ?? "").trim().toLowerCase());
            const roleB = (Array.isArray(b.role) ? b.role : [b.role]).map(r => String(r ?? "").trim().toLowerCase());
...
window.renderTablePengguna = () => {
    ...
    allUsersData.forEach((user) => {
        const roles = (Array.isArray(user.role) ? user.role : [user.role]).map(r => String(r ?? "").trim().toLowerCase()).filter(Boolean);
...
window.downloadDaftarPengguna = () => {
    ...
    const ws = XLSX.utils.json_to_sheet(data); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Daftar Pengguna"); XLSX.writeFile(wb, "Daftar_Pengguna_TRIGUNA_1956.xlsx");
};
